...
class Crazyflie:
    def __init__(self, prefix, tf):
        ...
        self.hover_setpoint_pub=rospy.Publisher(prefix+'/cmd_hover',Hover,queue_size=10)
        self.position_setpoint_pub=rospy.Publisher(prefix+'/cmd_position',Position,queue_size=10)
        self.full_state_setpoint_pub=rospy.Publisher(prefix+'/cmd_full_state',FullState,queue_size=10)
        self.velocity_setpoint_pub=rospy.Publisher(prefix+'/cmd_velocity_world',VelocityWorld,queue_size=10)
    ...
    def send_position_setpoint(self,x,y,z,yaw):
        ...
        p.z=z
        p.yaw=yaw
        self.position_setpoint_pub.publish(p)
    ...
    def send_hover_setpoint(self,vx,vy,yawrate,z):
        ...
        p.yawrate=yawrate
        p.zDistance=z
        self.hover_setpoint_pub.publish(p)
        
    def send_full_state_setpoint(self,r,v,a,rpy,rate):
        p=FullState()
        p.header.stamp = rospy.Time.now()
        p.pose.position.x=r[0]
        p.pose.position.y=r[1]
        p.pose.position.z=r[2]
        p.twist.linear.x=v[0]
        p.twist.linear.y=v[1]
        p.twist.linear.z=v[2]
        p.acc.x=a[0]
        p.acc.y=a[1]
        p.acc.z=a[2]
        ...
        p.twist.angular.x=rate[0]
        p.twist.angular.y=rate[1]
        p.twist.angular.z=rate[2]
        self.full_state_setpoint_pub.publish(p)
    ...
    def send_velocity_world_setpoint(self,vx,vy,vz,yawRate):
        ...
        p.vel.z=vz
        p.yawRate=yawRate
        self.velocity_setpoint_pub.publish(p)
